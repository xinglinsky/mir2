# Bevy（Rust）客户端完整重写计划（Windows x64 优先；预留 Mac/Web；KTX2 单包；点地寻路）
...
- **跨平台预留**：Windows x64 先上线，后续可上 Mac/Web（同一套代码 + 资源包）。
...
- **资源置换**：统一使用 **KTX2（BasisU）单包策略**（同一份内容包跨 Windows/Mac/Web）。
...
- **引擎/框架**：Bevy（ECS + wgpu）。
  - 理由：Rust 技术栈统一；渲染后端现代（D3D12/Metal/Vulkan/Web）；便于长期演进（ECS 组织复杂玩法与表现）。
- **语言/版本**：Rust（Edition 2021）。
- **运行目标**：
  - 桌面：Windows x64 / macOS。
  - Web：WASM（优先 WebGPU，必要时回退 WebGL2；以 Bevy/ wgpu 能力为准）。

### 1.2 资源格式

- **统一容器**：`content.cpack`（单文件 pack：index + offset + sha256）。
- **纹理**：KTX2（Basis Universal）。
  - **MVP 阶段**：允许 CPU 解码为 RGBA8，写入 Bevy `Image` 再上传 GPU（先跑通）。
  - **性能阶段**：接入 BasisU/平台转码，尽量走 GPU 压缩纹理：Windows→BCn，Mac→ASTC，Web→按浏览器支持选择（同一 KTX2 数据源）。

### 1.3 网络与协议

- **协议兼容**：优先复用现有 `Shared`/包定义（字节级兼容）。
- **网络 IO（桌面）**：Tokio（独立 runtime 线程）+ mpsc/channel 与 Bevy 主线程通信。
- **网络 IO（Web）**：WebSocket（WASM 环境）；同样通过 channel/事件桥接到 Bevy。
- **包处理预算**：每帧处理最多 N 个包（例如 200），避免洪峰卡顿。
...
- `crystal-client-bin/`
  - 可执行入口：Bevy `App` 初始化、插件组装、平台差异处理（Windows/macOS/Web）。
- `crystal-client-app/`
  - “应用层”：场景切换（Login/Select/Game）、全局状态、调试 overlay、配置。
- `crystal-client-render/`
  - 2D 渲染与相机：地图层/对象层/特效层/UI 层渲染编排；渲染统计。
- `crystal-client-content/`
  - `cpack` 读取、manifest、KTX2/atlas 元数据读取；Bevy `AssetLoader`/缓存策略。
- `crystal-client-net/`
  - 连接管理、收发、解包线程（桌面：Tokio；Web：WebSocket）、包队列与预算。
- `crystal-client-gameplay/`
  - 地图/对象/移动/聊天/战斗（后续）等“纯逻辑层”（尽量不依赖 Bevy 渲染）。
- `crystal-client-ui/`
  - Mir 风格 UI：优先用 Bevy UI 体系实现核心面板；复杂控件可评估引入 `bevy_egui` 做工具类 UI。
...
### 3.1 主线程（Bevy Schedules）

- `Update`（一组 systems）：
  - 消费网络包（限额）→ 驱动 `WorldState`/ECS 组件更新。
  - 输入处理：鼠标点地 → 寻路 → 生成 direction 队列。
  - 本地预测/插值：玩家与其他对象的平滑移动（以组件/资源形式维护）。
  - UI 更新：聊天输入、窗口交互。
- `Render`：
...
- pack 读取 + KTX2 解码可能较重：
  - MVP 阶段允许同步/小资源同步；
  - 后续改为：后台任务解码 + 主线程/渲染线程只提交 GPU 资源（按 Bevy Asset 管线落地）。
...
- **V0：Bevy 应用骨架**
  - Bevy `App` 跑通、开窗、输入、Debug overlay（FPS/队列长度/耗时）。
- **V1：网络连通 + 聊天闭环**
  - 桌面端 Tokio 连接/收发/解包线程；聊天发送与展示；包处理预算生效。
- **V2：登录 + 进图（地图可先占位）**
...
- 自定义 `AssetLoader`：从 `content.cpack` 按需读取 KTX2/atlas 元数据。
- 自定义 Asset 类型：
  - `AtlasManifest`（json）
  - `PackedKtx2`（原始压缩数据或转码后数据）
- 将纹理落到 Bevy `Image`：
  - MVP：填充 RGBA8。
  - 性能阶段：评估 Bevy/wgpu 对压缩纹理的支持路径（不同平台/后端能力不同），必要时做“平台降级策略”。

Web/WASM 预留要点

- WASM 环境建议走 WebSocket（浏览器限制下不走原始 TCP）。
- `content.cpack` 建议提供 HTTP Range/分块下载能力（或直接按 atlas 分包），让 Web 首屏更可控。
- 纹理压缩在 Web 端必须准备降级策略：
  - 浏览器支持压缩纹理扩展：优先使用。
  - 不支持：回退到 RGBA8（但要严控 atlas 常驻集）。
...
- 网络缓冲：复用接收/发送缓冲区（例如 `BytesMut`/ring buffer 思路），避免频繁分配。
- 逻辑临时对象：尽量复用 `Vec`（`clear()` 保留容量）、使用 `SmallVec` 处理小集合。
- ECS 写法约束：避免每帧大量创建/销毁实体与组件；对于短命特效用对象池/批量回收策略。
...
1) 建立 Rust workspace：`crystal-client-*` crate 骨架，并用 Bevy 跑通 V0（开窗/输入/调试 overlay）。
2) 在 RustServer 侧准备最小对接包清单（登录/StartGame/MapChanged/UserLocation/Chat/Walk/Run），并确定 Web 端是否需要 WebSocket 网关。
3) 同步推进 `tools-content-pipeline`：先做 UI atlas（最容易验证 KTX2/pack），并产出最小 `content.cpack`。
...
- 同层渲染按 atlas/material 分组，减少 wgpu pipeline/bind group 切换。
- mask/tint（若存在）优先通过 shader 实现，避免二次绘制导致 draw call 翻倍。

---

## 17. Bevy 实现细化（ECS 组件/系统划分）

本节用于把“计划”落到 Bevy 可直接实现的组件与系统。

### 17.1 核心 ECS 组件（示例）

- `NetId`：服务端对象 id/session id。
- `MapPos`：格坐标/像素坐标。
- `Facing`：方向（0-7）。
- `MoveQueue`：待发送的 direction 队列（点地寻路输出）。
- `AnimState`：站立/走/跑/攻击/受击/死亡。
- `NameTag`、`HpBar`、`BuffList`：UI 绑定数据。

### 17.2 关键系统（示例顺序）

- `net_pump_system`：每帧按预算消费网络包 → 写入 `WorldState`/触发事件。
- `input_click_to_move_system`：鼠标点地 → A* → 更新 `MoveQueue`。
- `movement_send_system`：按 tick 从 `MoveQueue` 取 direction 发送 `CWalk/CRun`。
- `movement_reconcile_system`：处理 `SUserLocation`/对象广播，做插值与纠偏。
- `spawn_despawn_system`：根据服务端事件创建/销毁实体，并处理资源引用计数。

---

## 18. 包体与性能优化（Bevy/Rust/WASM）

### 18.1 桌面端（Windows/macOS）

- Release 构建开启 LTO、strip 符号（按发布链路落地）。
- 精简依赖与 feature：仅启用 2D 所需能力，避免引入不必要的 3D/后处理模块。
- 纹理与资源体积主要靠 KTX2/atlas/分包控制；程序本体不必追求极限小，但需可控。

### 18.2 Web/WASM

- WASM 需要配合压缩（gzip/brotli）与缓存（Service Worker/HTTP cache）。
- 首屏资源必须可控：
  - 拆分 UI/登录必需 atlas 与游戏内 atlas。
  - 允许进入登录/选角时只加载 UI 资源。
- 注意浏览器内存限制：优先减少 atlas 常驻集与解码后的 RGBA8 峰值。
...
- 阶段 D：性能与跨平台（KTX2 GPU 转码 + Mac/Web）

---

## 20. 验收标准（建议写入 CI/QA 清单）

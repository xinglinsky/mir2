    ...
    public sealed class RelationshipDialog : MirImageControl
    {
        ...
        public RelationshipDialog()
        {
            ...
            RequestButton.Click += (o, e) =>
            {
                if (LoverName != "")
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.Relationship_AlreadyMarried, ChatType.System);
                    return;
                }
            ...
            DivorceButton.Click += (o, e) =>
            {
                if (LoverName == "")
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.Relationship_NotMarried, ChatType.System);
                    return;
                }
            ...
            WhisperButton.Click += (o, e) =>
            {
                ...
                if (MapName == "")
                {
                    GameScene.Scene.ChatDialog.ReceiveChat(GameLanguage.Relationship_LoverNotOnline, ChatType.System);
                    return;
                }
        ...
        {
            LoverNameLabel.Text = string.Format(GameLanguage.Relationship_LoverLabel, LoverName);

            if (MapName != "")
            {
                LoverOnlineLabel.Text = string.Format(GameLanguage.Relationship_Location, MapName);
            }
            else
                LoverOnlineLabel.Text = GameLanguage.Relationship_LocationOffline;

            if ((LoverName == "") && (Date != default))
            {
                if (Date < new DateTime(2000))
                {
                    LoverDateLabel.Text = GameLanguage.Relationship_Date;
                    LoverLengthLabel.Text = GameLanguage.Relationship_Length;
                }
                else
                {
                    LoverDateLabel.Text = string.Format(GameLanguage.Relationship_DivorcedDate, Date.ToShortDateString());
                    LoverLengthLabel.Text = string.Format(GameLanguage.Relationship_TimeSinceDays, MarriedDays);
                }


                LoverOnlineLabel.Text = GameLanguage.Relationship_Location;
                AllowButton.Hint = GameLanguage.SwitchMarriage;
            }
            else
            {
                LoverDateLabel.Text = string.Format(GameLanguage.Relationship_MarriageDate, Date.ToShortDateString());
                LoverLengthLabel.Text = string.Format(GameLanguage.Relationship_LengthDays, MarriedDays);
                AllowButton.Hint = GameLanguage.Relationship_AllowBlockRecall;
            }
